...
| Team Ground Rule | 회의는 가능한 한 30분 이내로 간결하게 진행하는 것을 원칙으로 한다. 회의를 효율적으로 진행하기 위해 회의 시작 1시간 전까지 각자 진행 상황과 안건을 미리 작성한다. 작성 내용에는 현재까지의 진행 사항과 이후 진행 예정 사항을 포함한다. 또한 회의에 참석하기 전에는 다른 파트에서 공유한 진행 사항을 미리 확인하고 참여하여 회의 시간을 효율적으로 사용할 수 있도록 한다. 프로젝트 진행 중 특정 주제에 대한 논의가 필요한 경우에는 하나의 토픽에 대해 별도의 스레드를 생성한 뒤 관련 파트를 언급하여 소통한다. 업무 요청이 필요한 경우에는 요청 내용, 요청 배경, 담당 파트, 데드라인을 명확히 포함하여 전달한다. 이를 통해 요청의 맥락과 우선순위를 모두가 쉽게 이해할 수 있도록 한다. 또한 팀원 간 원활한 협업을 위해 카카오톡 또는 디스코드 메시지는 가능하면 2시간 이내에 확인하고 응답하는 것을 기본 원칙으로 한다. |
